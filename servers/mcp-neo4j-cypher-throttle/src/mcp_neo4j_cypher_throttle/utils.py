...
def process_config(args: argparse.Namespace) -> dict[str, Union[str, int, None]]:
    ...
    if args.query_timeout is not None:
        ...
        if os.getenv("QUERY_TIMEOUT") is not None:
            try:
                config["query_timeout"] = float(os.getenv("QUERY_TIMEOUT"))
            except ValueError:
                logger.warning(
                    "Warning: QUERY_TIMEOUT environment variable is not a valid integer. Using default: 10"
                )
                config["query_timeout"] = 10
        else:
            logger.warning(
                "Warning: No query timeout provided. Using default: 10"
            )
            config["query_timeout"] = 10
